...
public class EliminarHospitales extends JFrame {
    ...
    public EliminarHospitales() {
        ...
        tableModel = new DefaultTableModel(new Object[]{"ID Hospital", "Nombre", "Tipo", "ID Ciudad"}, 0);
    ...
    private void cargarDatosTabla() {
        try {
            ...
            while (rs.next()) {
                ...
                tableModel.addRow(new Object[]{idHosp, nombre, tipo, idCiudad});
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private void eliminarRegistro() {
        ...
        if (filaSeleccionada != -1) {
            String idHosp = tableModel.getValueAt(filaSeleccionada, 0).toString();
            try {
                String query = "DELETE FROM hospitales WHERE idHosp=?";
                PreparedStatement pstmt = conn.prepareStatement(query);
                pstmt.setString(1, idHosp);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new EliminarHospitales().setVisible(true);
            }
        });
    }
}
